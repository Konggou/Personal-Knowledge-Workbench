...
- 先做前后端源码主干的模块化冗余审查，不按零散文件逐个修补
- 第一批只处理高置信、低风险、低行为变化的冗余：
  - 死代码删除
  - 重复 helper 合并
  - 旧产品心智残留清理
- 同步建立 V6-1 审查清单、文档差异清单与后续分批实施说明
- 结构级重复逻辑收敛单列到后续批次，不与本批混做
- 当前状态：
  - V6-1 已完成，已产出审查矩阵、文档差异清单与批次拆分
  - 高置信代码去冗余与文档事实冲突已完成修正
  - 更高风险的结构级收敛留在后续批次
...
- 新增 `/settings` 页面，统一配置：
  ...
  - `Embedding`
  - `Reranker`
- 配置写入 SQLite `_app_metadata`，并作为运行时优先配置源
- 环境变量继续保留为默认回退
- API Key 仅显示掩码与配置状态，支持更新与清空
