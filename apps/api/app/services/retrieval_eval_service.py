        ...
        results.append(
            {
                **asdict(case),
                ...
                "lexical_hit_count": sum(1 for item in evidence if item.get("lexical_rank")),
                "semantic_hit_count": sum(1 for item in evidence if item.get("semantic_rank")),
                "fused_hit_count": len(evidence),
                ...
                "fused_top_score": diagnostics.get("effective_pass", {}).get("top_score", diagnostics["first_pass"]["top_score"]),
                "rerank_backend": diagnostics.get("rerank", {}).get("backend"),
